#!/usr/bin/env bun
/**
 * Variant-Specific Audit Hook (co-abap-plugin)
 * @version 1.0.0
...
if (failures.length > 0) {
  console.log(`❌ co-abap-plugin variant audit: ${failures.length} check(s) failed`);
  process.exit(1);
}
console.log("✅ co-abap-plugin variant audit: all checks passed");
