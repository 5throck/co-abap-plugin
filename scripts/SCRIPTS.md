...
> **Bun is a workspace prerequisite.** Install it once via the [official installer](https://bun.sh/docs/installation) before using any script. `install-bun.sh/ps1` were deleted from the workspace (ADR-0036); this plugin repo retains thin `scripts/install-bun.{sh,ps1}` and `install-vsp.{sh,ps1}` consumer wrappers (see README).
