...
## [Unreleased]
- **[2026-09-26]**: docs(setup): add Bun to the `docs/plugin-setup.md` prerequisites — §2-B's automated hook (`bun "$CLAUDE_PLUGIN_ROOT/scripts/sync-md.ts"`) and the dev-sync utilities require the Bun runtime, but the prerequisites list never mentioned it (found during the bun/bunx default-availability review; install commands mirror the README).
- **[2026-09-26]**: fix(hooks): drop `${CLAUDE_PROJECT_DIR:-.}` shell expansion from the statusLine command in `.claude/settings.json` — the same POSIX-only syntax removed from the graft PostToolUse hooks in b46a2da, missed there because the statusLine entry lives outside the `hooks` block. Plain relative command now (cwd is the project root; the helper resolves `CLAUDE_PROJECT_DIR` internally).
- **[2026-09-26]**: fix(mcp): drop the stray `@nanonets/graft` argument from the graft server entry in `.gemini/settings.json` — a regression reintroduced by the 2026-09-26 workspace sync (commit b46a2da^): the Sep-20 fix f4cc29a had cleaned `.mcp.json`, but the sync overwrote `.gemini/settings.json` with the co-abap copy that still carried the stale form. `graft mcp` reads the second argument as a repository dir, so Gemini CLI sessions got a graft stdio server that failed to resolve this repo's graph. Back to the clean `["mcp"]` form, matching `.mcp.json` and `.agents/mcp.json`.
- **[2026-09-26]**: fix(plugin): cross-platform packaging fixes from the Windows/macOS/Linux support audit. `.claude-plugin/plugin.json` — removed the dangling `"hooks": "./hooks/hooks.json"` reference (the file was deleted 2026-07-08 when hooks moved to consumer-side settings; every platform hit it at plugin load) and switched the abap MCP server to the documented plugin-bundle form `"${CLAUDE_PLUGIN_ROOT}/vsp"` — the previous bare `vsp` PATH lookup never matched either documented install location, so consumers following the README or plugin-setup flows got no working MCP server. `docs/plugin-setup.md` — vsp prerequisite corrected to plugin-root install via `scripts/install-vsp.{sh,ps1}` (was the unreachable `~/abap` location). `.claude/settings.json` — graft hook commands no longer rely on `${CLAUDE_PROJECT_DIR:-.}` shell expansion and the sync-md hook drops `2>/dev/null || true` (POSIX-only syntax that fails under cmd.exe/PowerShell; the helper resolves `CLAUDE_PROJECT_DIR` internally). `.mcp.json.sample` — header now states the three `abap-*` entries are per-OS alternatives to keep exactly one of (an as-is copy launched three duplicate vsp servers) and that `./vsp` expects the binary at the project root. `ci.yml` — audit job now runs a `fail-fast: false` matrix over ubuntu/macos/windows so packaging regressions surface in CI (secret-scan stays ubuntu-only: docker-based gitleaks). Design: `docs/designs/2026-09-26-cross-platform-plugin-fixes-design.md`.
- **[2026-09-26]**: chore(sync): reflect the co-abap workspace (2026-09-26 state, waves #144–#160) into the plugin repo. **Agents**: all 21 definitions refreshed to the W4 thin-dispatcher format (ADR-0090) and `i18n-specialist` added. **Skills**: all refreshed; `design-foundation` added; `handbook`/`explain-me` now carry their assets/examples/references/templates sub-dirs; mirrored across `.agents`/`.claude`/`.codex`/`.gemini` plus the new fifth `.hermes` mirror (ADR-0088). **Docs**: `AGENTS.md` regenerated thin-dispatcher body, ADR-0002, `docs/governance/agents/` workflow docs, lifecycle records (+7 new), context pair, user guides, country-profiles, deliverables templates 05/06 promoted. **Scripts**: full overlay including `scripts/lib/{platforms,upgrade-policy,dependency-guard}.ts`, `regenerate-agents-md.ts`, `review-baseline.ts`, `co-abap/atc-rulepack.json`; plugin-only `install-*.{sh,ps1}`, `scripts/package.json`, `helpers/generate-variant.ts` kept and re-registered in `SCRIPTS.md` (144 scripts verified). **Security posture adopted from upstream**: `.env.sample` example password redacted; committed `SAP_FEATURE_*` defaults flipped to `off` in `.claude/settings.json`/`.codex/config.toml`/`.gemini/settings.json` (plugin.json consumer flags unchanged); `.gitleaks.toml` now scans `memory/` per upstream policy (verified clean locally), drops the co-abap-specific `DEVELOPER:Down1oad` stopword, adds the `scratch/` exclusion. **Plugin-local preserved**: CLAUDE.md Setup/Consumer Integration sections, `.claude/settings.json` sync-md hook, `.codex/hooks.json` SessionStart design, `.gitignore` extras (`.mcp.json`, `bun.lockb`), plugin READMEs (counts refreshed to 21 agents / 47 skills), `docs/specs/registry.json`, `CHANGELOG.md`, `commands/`, `config/`, `.claude-plugin/`. Gates: `audit.ts` PASS, `verify-scripts.ts --verify` 144 OK, agent/skill validators PASS, `gitleaks detect` clean. Design: `docs/designs/2026-09-26-co-abap-sync-design.md`.
...
- **[2026-08-21]**: chore(upgrade+governance): synced template infrastructure via `upgrade-project.ts` (`docs/context.md` v2.1 → v2.4) and backfilled a substantial pre-existing governance gap surfaced by the sync. **Template sync**: `.gitleaks.toml` merged (correctly found zero project-only entries to preserve), 104 script/skill/doc sync operations. **Governance backfill (pre-existing, unrelated to the template sync itself)**: all 21 `agents/*.md` files were missing `lifecycle.phase`/`lifecycle.governance` frontmatter entirely — added `lifecycle: {phase: production, created, last_updated, governance: docs/lifecycle/agents/<name>.md}` to each (`bun scripts/validate-agents.ts` now passes with 0 errors; the `docs/lifecycle/agents/` governance-record directory itself is left absent, matching the same warn-only, frontmatter-first pattern already established elsewhere in this template family, e.g. `co-game`). `scripts/SCRIPTS.md` didn't exist at all — generated via `verify-scripts.ts --generate` then corrected 51 version placeholders against each file's real `@version` header (the generator defaults everything to `1.0.0`); registered `new-requirement.test.ts` and added its missing `@version` header. Moved `CONTRIBUTING.md`/`PR_BODY.md` from the project root to `docs/` (File Organization Policy — non-standard root `.md` files are prohibited; `PR_BODY.md` in particular looked like stale scratch content from a prior PR draft, kept rather than deleted since its provenance wasn't fully clear). `bun scripts/audit.ts` and `gitleaks detect` both pass clean.
- **[2026-08-18]**: chore(align): align agents, skills, and scripts with `templates/co-abap` (canonical variant template). **Agents (20 files)**: replaced legacy-format agent definitions with the current template versions (governance frontmatter `status`/`tier`, PM-ONLY INVOCATION sections, Activation Instructions / Output Format structure); `pm.md` now uses the resolved variant version (extends the workspace `templates/common/agents/pm.md` chain, matching the scaffolded co-abap project); added `agents/README.md`/`README_ko.md`. **Skills (12 SKILL.md)**: updated to template versions with full lifecycle frontmatter (version, last_reviewed, owner, prerequisites). **Scripts**: updated 8 variant scripts (dispatch trio, new-requirement, retry-handler 1.0.2, setup, vsp-publish, vsp-task) and added 4 new ones (install-bun.ts, install-vsp.ts, scratch-cleanup.ts, vsp-audit.ts); restructured the ADR-0050 dispatch wrappers — common logic now at `scripts/dispatch-{parallel,serial}.ts`, variant wrappers at `scripts/co-abap/dispatch*.ts` (`package.json` and `AGENTS.md` references updated). **Fixed a pre-existing template bug surfaced during alignment**: the variant dispatch wrappers' import path `../../dispatch-*.ts` is wrong for every scaffolded layout (resolves outside the project) — corrected to `../dispatch-*.ts` in this project's copies; the template itself still carries the bug. Updated the Scripts table in `docs/co-abap.context.md` to match. `bun scripts/audit.ts`, `validate-docs-links.ts`, `agent-verify.ts` all pass.
